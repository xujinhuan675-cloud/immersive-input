...
static LAST_USER_COPY_MS: AtomicU64 = AtomicU64::new(0);
    ...
    match key {
        ...
        Key::KeyC | Key::Insert => {
            if copy_modifier_active() && !internal_copy_active() {
                LAST_USER_COPY_MS.store(current_marker(), Ordering::SeqCst);
            }
        }
        _ => {}
    }
...
fn has_user_copy_since(marker: Option<u64>) -> bool {
    marker
        .map(|value| LAST_USER_COPY_MS.load(Ordering::SeqCst) >= value)
        .unwrap_or(false)
}
...
fn get_text_windows(user_copy_priority_marker: Option<u64>) -> String {
    if let Some(text) = read_user_clipboard_if_needed(user_copy_priority_marker) {
        return text;
    }
...
#[cfg(target_os = "windows")]
fn read_user_clipboard_if_needed(user_copy_priority_marker: Option<u64>) -> Option<String> {
    use std::time::Duration;

    if !has_user_copy_since(user_copy_priority_marker) {
        return None;
    }
...
fn get_text_by_clipboard(
    ...
    let seq_after_capture = unsafe { GetClipboardSequenceNumber() };
    ...
    let seq_before_restore = unsafe { GetClipboardSequenceNumber() };
