...
自定义 AI 服务属于 BYOK（用户自带 API Key），不消耗 Sub2API 的订阅额度或余额，免费版、基础版、专业版和企业版都可以配置自定义兼容接口。

未选择自定义服务，或将 URL 保持为 FlowGuide 网关地址时，文本 AI 和语音 AI 才使用 Sub2API 网关，并按当前订阅、余额策略计费。自定义 URL 如果没有同时填写用户自己的 API Key，会被配置校验拦截，不会由平台代付。前端不能只依靠套餐等级隐藏自定义配置，真实请求层也必须保留自定义地址。
