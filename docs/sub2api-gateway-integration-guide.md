...
默认情况下，业务项目的文本 AI 和语音 AI 都应走 Sub2API 网关。即使本地历史配置里仍保存第三方 `API URL` 或 `API Key`，非专业权益用户在运行时也必须回落到网关地址，并使用当前登录态鉴权。

只有当 Sub2API 返回的当前有效套餐明确属于专业/企业权益时，业务项目才可以开放自定义 AI 服务，也就是用户自带 API Key（BYOK）或自定义兼容接口。推荐判断信号：

- 套餐或用户归一化 `tier` 为 `pro`、`enterprise`。
- 套餐名、产品名或分组名包含 `Pro`、`Plus`、`Premium`、`Professional`、`专业`、`高级`、`企业`、`团队`。

前端 UI 可以隐藏或禁用自定义服务编辑入口，但真实约束必须落在共享的 AI 配置解析层或请求发送层，避免其他入口绕过。

示例：

- 免费版、基础版：`/v1/chat/completions` 和 `/v1/audio/speech` 都使用 Sub2API 网关。
- 专业版：默认仍使用 Sub2API 网关；用户主动配置自定义服务后，可使用自定义 OpenAI-compatible 地址。
